...
export const register = createAsyncThunk('auth.slice/register', async data => await api.post('register/', data));
...
const auth = createSlice({
    ...
    extraReducers: ({ addCase }) => { // used for updating the states when fetching an api data and error handling
        ...
        addCase(register.rejected, (state, { error }) => { state.load = false; console.error(error.message); });
    },
});
